...
function productSoldQtyByProductId($con,$pid){
	$sql="select sum(order_detail.qty) as qty from order_detail,`order_document` where `order_document`.id=order_detail.order_id and order_detail.product_id=$pid and `order_document`.order_status!=4";
...
function placeShipRocketOrder($con,$token,$order_id){
	$row_order=mysqli_fetch_assoc(mysqli_query($con,"select `order_document`.*,users.name,users.email,users.mobile from `order_document`,users where `order_document`.id=$order_id and `order_document`.user_id=users.id"));
  ...
  mysqli_query($con,"update `order_document` set ship_order_id='$ship_order_id', ship_shipment_id='$ship_shipment_id' where id='$order_id'");
