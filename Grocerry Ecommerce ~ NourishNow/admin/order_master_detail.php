...
$coupon_details=mysqli_fetch_assoc(mysqli_query($con,"select coupon_value,coupon_code from `order_document` where id='$order_id'"));
...
if(isset($_POST['update_order_status'])){
	...
	if($update_order_status=='5'){
		mysqli_query($con,"update `order_document` set order_status='$update_order_status',payment_status='Success' where id='$order_id'");
	}else{
		mysqli_query($con,"update `order_document` set order_status='$update_order_status' $update_sql where id='$order_id'");
	}
	...
	if($update_order_status==4){
		$ship_order=mysqli_fetch_assoc(mysqli_query($con,"select ship_order_id from `order_document` where id='$order_id'"));
				   ...
				   <div class="table-stats order-table ov-h">
					  <table class="table">
								...
								<tbody>
									...
									$userInfo=mysqli_fetch_assoc(mysqli_query($con,"select * from `order_document` where id='$order_id'"));
						...
						<div id="address_details">
							...
							<?php 
							$order_status_arr=mysqli_fetch_assoc(mysqli_query($con,"select order_status.name,order_status.id as order_status from order_status,`order_documentr` where `order_document`.id='$order_id' and `order_document`.order_status=order_status.id"));
